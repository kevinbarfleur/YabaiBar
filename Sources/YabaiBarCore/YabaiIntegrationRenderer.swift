...
public enum YabaiIntegrationRenderer {
    public static let startMarker = "# >>> YabaiBar >>>"
    public static let endMarker = "# <<< YabaiBar <<<"
    ...
    public static func render(
        ...
        return ([startMarker, "# Managed by YabaiBar. Changes inside this block will be replaced."]
            + lines
            + [endMarker, ""])
            .joined(separator: "\n")
    }
    ...
    private static func managedRange(in contents: String) -> Range<String.Index>? {
        guard let start = contents.range(of: startMarker)?.lowerBound else {
            return nil
        }

        guard let endMarkerRange = contents.range(of: endMarker, range: start..<contents.endIndex) else {
            return nil
        }
