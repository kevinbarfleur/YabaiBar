...
struct SettingsView: View {
        ...
        List(selection: $selectedItem) {
            ...
            Section("Modules") {
                ForEach(model.moduleRegistry.modules, id: \.identifier) { module in
                    ...
                    if module.identifier.rawValue == "com.vibenotch.yabai",
                       model.moduleRegistry.isEnabled(module.identifier) {
                        Label("Diagnostics", systemImage: "stethoscope")
                            .padding(.leading, 12)
                            .tag(SettingsSidebarItem.moduleSubpage("com.vibenotch.yabai", "diagnostics"))
                        Label("Configuration", systemImage: "doc.text")
                            .padding(.leading, 12)
                            .tag(SettingsSidebarItem.moduleSubpage("com.vibenotch.yabai", "configuration"))
                    }
                }
            }
        }
    ...
    private func moduleSettingsPage(for moduleID: String) -> some View {
        ...
        return Form {
            ...
            if isEnabled, let settingsView = module?.makeSettingsView() {
                settingsView
            }
        }
    }
    ...
    private func moduleSubpage(for moduleID: String, subpage: String) -> some View {
        if moduleID == "com.vibenotch.yabai" {
            switch subpage {
            case "diagnostics":
                diagnosticsPage
            case "configuration":
    ...
    private static func parseYabaiSettings(_ content: String) -> [(key: String, value: String)] {
        ...
        let managedMarkers = ["# >>> VibeNotch >>>", "# >>> YabaiBar >>>"]
        let managedEndMarkers = ["# <<< VibeNotch <<<", "# <<< YabaiBar <<<"]
        var inManaged = false
