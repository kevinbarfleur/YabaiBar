...
@MainActor
final class TodoListModule: ObservableObject, VibeNotchModule {
    let identifier = ModuleIdentifier("com.vibenotch.todolist")
    ...
    @Published var fontSize: CGFloat {
        didSet {
            UserDefaults.standard.set(fontSize, forKey: DefaultsKey.fontSize)
            objectDidChange?()
        }
    }
    ...
    @Published var showCompleted: Bool {
        didSet {
            UserDefaults.standard.set(showCompleted, forKey: DefaultsKey.showCompleted)
            objectDidChange?()
        }
    }
    ...
    private enum DefaultsKey {
        ...
        static let fontSize = "TodoList.fontSize"
        static let showCompleted = "TodoList.showCompleted"
    }

    init() {
        let defaults = UserDefaults.standard
        fontSize = defaults.object(forKey: DefaultsKey.fontSize) as? CGFloat ?? 10
        showCompleted = defaults.object(forKey: DefaultsKey.showCompleted) as? Bool ?? true
        loadItems()
    }
    ...
    func updateItemTitle(_ id: UUID, _ newTitle: String) {
        guard let index = items.firstIndex(where: { $0.id == id }) else { return }
    ...
    func makeSettingsView() -> AnyView? {
        AnyView(TodoListSettingsView(module: self))
    }
}
