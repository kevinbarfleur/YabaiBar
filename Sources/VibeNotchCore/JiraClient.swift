...
public enum JiraClient {
    ...
    private static let keychainService = "VibeNotch-Jira"
    ...
    /// Migrate credentials from the old "OpenNotch-Jira" keychain entry if present.
    public static func migrateFromOldKeychainIfNeeded() {
        // Skip if new credentials already exist
        if readCredentials() != nil { return }

        let oldQuery: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: "OpenNotch-Jira",
            kSecAttrAccount as String: keychainAccount,
        ...
        let creds = JiraCredentials(siteUrl: siteUrl, email: email, apiToken: apiToken)
        if saveCredentials(creds) {
            NSLog("VibeNotch: migrated Jira credentials from OpenNotch keychain")
        }
    }
