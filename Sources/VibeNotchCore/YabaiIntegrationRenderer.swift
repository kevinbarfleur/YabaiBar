...
public enum YabaiIntegrationRenderer {
    public static let startMarker = "# >>> VibeNotch >>>"
    public static let endMarker = "# <<< VibeNotch <<<"
    public static let legacyStartMarker = "# >>> YabaiBar >>>"
    public static let legacyEndMarker = "# <<< YabaiBar <<<"
    public static let legacyOpenNotchStartMarker = "# >>> OpenNotch >>>"
    public static let legacyOpenNotchEndMarker = "# <<< OpenNotch <<<"
    ...
    public static func render(
        ...
        return ([startMarker, "# Managed by VibeNotch. Changes inside this block will be replaced."]
            + lines
            + [endMarker, ""])
            .joined(separator: "\n")
    }
    ...
    private static func managedRange(in contents: String) -> Range<String.Index>? {
        ...
        } else if let openNotchStart = contents.range(of: legacyOpenNotchStartMarker)?.lowerBound {
            resolvedStart = openNotchStart
            resolvedEndMarker = legacyOpenNotchEndMarker
